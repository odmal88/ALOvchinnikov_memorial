(function initContentSync() {
  ...
  function syncRouteMap(routeData) {
    ...
    const activeLabel = section.querySelector('.artist-route-map-active-label');
    ...
    if (!map || !pointsWrap || !legendWrap || !activeLabel || !captionTitle || !captionText) return;
    ...
    function setActivePoint(pointId) {
      ...
      const leftPercent = Math.max(8, Math.min(92, (active.projectedX / 420) * 100));
      const topPercent = Math.max(10, Math.min(92, (active.projectedY / 300) * 100));
      activeLabel.textContent = active.shortLabel || active.title || '';
      activeLabel.style.left = `${leftPercent}%`;
      activeLabel.style.top = `${topPercent}%`;
    ...
    points.forEach((point, index) => {
      ...
      button.style.left = `${(point.projectedX / 420) * 100}%`;
      button.style.top = `${(point.projectedY / 300) * 100}%`;
