(function initWorksRuntime() {
    ...
    function fallbackParseSizeAndTechnique(rawSize, title, editorialNote) {
    ...
    function fallbackInferCategory(record) {
    ...
    function normalizeCatalog(catalog, imageMap, runtimeMap) {
        ...
        console.warn('OvchinnikovWorksModel is not loaded. Works runtime is using local fallback normalization.');
        const usedSlugs = new Set();
        const runtimeWorks = runtimeMap && runtimeMap.works ? runtimeMap.works : {};

        return Array.isArray(catalog)
            ? catalog.filter(fallbackIsPrimaryCatalogRecord).map((record) => {
                const id = String(record.id);
                const imageMeta = imageMap[id] || {};
                const runtimeEntry = runtimeWorks[id] || {};
                const parsed = fallbackParseSizeAndTechnique(record.size, record.title, record.editorialNote);
                const slug = makeUniqueSlug(runtimeEntry.slug || record.slug, record.title, record.id, usedSlugs);
                const category = categoryLabels[runtimeEntry.category] ? runtimeEntry.category : fallbackInferCategory(record);
                ...
                return {
                    ...
                    descriptionHtml: fallbackDescriptionHtml(record)
                };
            })
            : [];
    }
