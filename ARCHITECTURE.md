...
works-runtime.js — текущий runtime-слой каталога работ.
```
...
`bootstrap.js` загружает HTML-компоненты, страницы, `content-sync.js`, затем остальные runtime-скрипты. `content-sync.js` синхронизирует уже существующие DOM-узлы с JSON. `app.js` отвечает за поведение, роутинг и интерактив. `works-runtime.js` сейчас рендерит каталог и страницы произведений из JSON.
...
Каталог уже частично вынесен в JSON и рендерится через `works-runtime.js`, но похожая логика ещё остаётся в `app.js`. Следующий технический шаг — свести нормализацию произведений к одному месту, чтобы `app.js` и `works-runtime.js` не вели две параллельные системы.
