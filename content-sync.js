(function initContentSync() {
  ...
  function syncAbout(about) {
    ...
    if (Array.isArray(about?.biography?.paragraphs)) {
      document.querySelectorAll('#artist-family .artist-family-body p.dark:not(.artist-family-intro)').forEach((p, idx) => {
    ...
    if (Array.isArray(about?.themes?.items)) {
      ...
      if (themeContainer) {
        const introText = about?.intro?.headline || '';
        themeContainer.innerHTML = [
          `<p class="dark artist-index-intro">${introText}</p>`,
