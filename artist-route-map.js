(function initArtistRouteMap() {
  ...
  function renderMap(route, worldAtlas, lakesJson, riversJson) {
    ...
    const countriesObject = worldAtlas?.objects?.countries || null;
    const countries = countriesObject ? topojson.feature(worldAtlas, countriesObject) : { type: 'FeatureCollection', features: [] };
    const landFallback = worldAtlas?.objects?.land ? topojson.feature(worldAtlas, worldAtlas.objects.land) : null;
    const countryBorders = countriesObject ? topojson.mesh(worldAtlas, countriesObject, (a, b) => a !== b) : null;
    const russiaFeature = getRussiaFeature(countries) || landFallback;
    ...
    mapLayer.append('g')
      ...
      .selectAll('path')
      .data(countries.features.length ? countries.features : (landFallback ? [landFallback] : []))
      .join('path')
